	...
	class dt {
		...
		*/
		...
		public function check($checks) {
			...
			// This if statement is for those values
			// That need checking if but does not contain checking for 'if_empty'
			// That is, the field is not compulsory, but if there is value, check
			if (!in_array("if_empty", $checks) && empty($this->data)) { 
				// No need to continue if field is empty
				return;
			}

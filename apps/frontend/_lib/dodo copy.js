...
function dd(selector) {
    ...
    self.clone = function(repeat) {
        ...
        for (var j = 1; j <= repeat; j++) {
            ...
            var ele = clone.attributes;
            for (var k = 0; k < ele.length; k++) {
                ele[k].value = ele[k].value.replace(/\$id/g, id);
            }
    ...
    self.feedback = function(feedback) {
        var target = self.target;
            ...
            if (data.hasOwnProperty(x)) {
                ...
                function checkAttr(attr) {
                    ...
                    for (var j = 0; j < ele.length; j++) {
                        if (ele[j].value.includes("["+x+"]")) {
                            var match = new RegExp("\\["+x+"\\]", "g");
                            ele[j].value = ele[j].value.replace(match, data[x]);
                        }
                    }
                }
                ...
                function checkFor(condition) {
                    ...
                    if (dat == x) {
                        ...
                        if (dd_if !== null && dd_if == data[x] || dd_if=='not_empty' && !dd(data[x]).isEmpty()) {
                            dd(q).show();
                        } else {
                            dd(q).hide();
                        } 
...
function dd_setSubmit() {
	...
	for (var i=0; i < forms.length; i++) {
        ...
        var method = forms[i].getAttribute('method') || 'POST';
...
function dd_submit(get) {
    ...
    self.redirect = get.redirect || null;
    self.dataTarget = get.bindData || '';
    self.resultTarget = get.bindResult || '';
    self.validate = get.validate || function() {
        return true;
    };
    self.data = '';
    ...
    self.__construct = function() {
        ...
        self.target.addEventListener('submit', function(e) {
            ...
            e.preventDefault();
            var validateFeedback = self.validate();
            ...
            if (validateFeedback === true) {
                ...
                dd(self.target).feedback(validateFeedback);
            }
...
function dd_init() {
    ...
    // We also have to set current page
    var current_url = window.location.pathname.slice(1);
    current_url = current_url.split('/');
    current_url = current_url[current_url.length-1];
    
    var active_link = dd("a[href='"+current_url+"']");

    if (active_link.validQuery()) {
        active_link.select().id = "dd_active";
    }
    ...
    window.addEventListener('DOMContentLoaded', function() {
        if (typeof dd_initialize === 'function') {  
            dd_initialize();
        }
    });
}
